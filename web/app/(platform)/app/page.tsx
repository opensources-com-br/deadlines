...
export default async function PlatformPage() {
  ...
  if (!accessToken) redirect(refreshToken ? "/api/auth/refresh?returnTo=/app" : "/login");
  ...
  if (!userResponse?.ok) redirect(refreshToken && userResponse?.status === 401 ? "/api/auth/refresh?returnTo=/app" : "/login");
