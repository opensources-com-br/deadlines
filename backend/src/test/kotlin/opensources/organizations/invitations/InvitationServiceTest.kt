package opensources.organizations.invitations

import opensources.config.EmailConfig
import opensources.identity.email.EmailTokenGenerator
import opensources.identity.email.EmailDeliveryException
import opensources.identity.email.EmailService
import opensources.identity.email.RecordingEmailService
...
import opensources.organizations.access.MemoryRoleRepository
...
import opensources.organizations.members.MemberRepository
import opensources.organizations.members.OrganizationMember
...
class InvitationServiceTest {
    ...
    private fun organizationContext(userId: UUID, id: UUID, owner: Boolean): OrganizationContext =
        opensources.organizations.access.accessContext(
            userId,
            id,
            if (owner) opensources.organizations.MembershipRole.OWNER else opensources.organizations.MembershipRole.MEMBER,
        )
...
private class FailingEmailService : EmailService {
    override suspend fun send(message: opensources.identity.email.EmailMessage): Nothing = throw EmailDeliveryException()
}
