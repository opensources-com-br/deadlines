...
import opensources.identity.auth.SessionRepository
import opensources.identity.users.UserCredentialsRepository
import java.time.Clock
import java.util.UUID

class InvalidPasswordResetTokenException : opensources.shared.errors.ApiException(
    status = 400,
    code = "INVALID_PASSWORD_RESET_TOKEN",
    message = "Password reset token is invalid or expired",
)

class PasswordResetValidationException : opensources.shared.errors.ApiException(
    status = 422,
